...
import { ServerOrderResponseSchema, ServerOrderResponseType } from '@/backend/database/schemas';
import { HttpStatus, APIResponse } from '@/backend/types';
...
import { MpRedirect, PreferenceMPValidator } from '@/features/payment';
...
export async function POST(request: Request): Promise<NextResponse<APIResponse<MpRedirect | Error>>> {
  try {
    const body: unknown = await request.json()
    const order: ServerOrderResponseType = ServerOrderResponseSchema.parse(body)
    ...
    return NextResponse.json<APIResponse<MpRedirect>>({
      success: true,
