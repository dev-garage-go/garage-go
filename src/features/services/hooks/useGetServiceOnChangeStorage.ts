...
export const useGetServiceOnChangeStorage = (): ServicesDataType | null => {
  ...
  // obtain the 'service' from local storage, if exist returns it, otherwise return null
  ...
  useEffect(() => {
    ...
    // Important: this event will shotting in serviceDataModal -> setservice(value: string)
    // Important: the function that will shoting this event it's in -> context/serviceContext.tsx
